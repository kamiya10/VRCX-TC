...
(async function () {
    ...
    $app.methods.playNoty = function (json) {
        ...
        switch (noty.type) {
            case 'OnPlayerJoined':
                text = `<strong>${noty.displayName}</strong> 加入了房間`;
                break;
            case 'OnPlayerLeft':
                text = `<strong>${noty.displayName}</strong> 離開了房間`;
                break;
            case 'OnPlayerJoining':
                text = `<strong>${noty.displayName}</strong> 正在加入房間`;
                break;
            case 'GPS':
                text = `<strong>${
                    noty.displayName
                }</strong> 正在 ${this.displayLocation(
                    noty.location,
                    noty.worldName
                )}`;
                break;
            case 'Online':
                ...
                text = `<strong>${noty.displayName}</strong> 已上線${locationName}`;
                break;
            case 'Offline':
                text = `<strong>${noty.displayName}</strong> 已離線`;
                break;
            case 'Status':
                text = `<strong>${noty.displayName}</strong> 的狀態現在是 <i>${noty.status}</i> ${noty.statusDescription}`;
                break;
            case 'invite':
                text = `<strong>${
                    noty.senderUsername
                }</strong> 邀請你到 ${this.displayLocation(
                    noty.details.worldId,
                    noty.details.worldName
                )}${message}`;
                break;
            case 'requestInvite':
                text = `<strong>${noty.senderUsername}</strong> 向你請求了邀請 ${message}`;
                break;
            case 'inviteResponse':
                text = `<strong>${noty.senderUsername}</strong> 回覆了你的邀請 ${message}`;
                break;
            case 'requestInviteResponse':
                text = `<strong>${noty.senderUsername}</strong> 回覆了你的邀請請求 ${message}`;
                break;
            case 'friendRequest':
                text = `<strong>${noty.senderUsername}</strong> 向你傳送了好友邀請`;
                break;
            case 'Friend':
                text = `<strong>${noty.displayName}</strong> 現在是你的朋友了`;
                break;
            case 'Unfriend':
                text = `<strong>${noty.displayName}</strong> 不再是你的朋友了`;
                break;
            case 'TrustLevel':
                text = `<strong>${noty.displayName}</strong> 的信認等級現在是 ${noty.trustLevel}`;
                break;
            case 'DisplayName':
                text = `<strong>${noty.previousDisplayName}</strong> 變更了他的名稱為 ${noty.displayName}`;
                break;
            case 'PortalSpawn':
                text = '用戶開啟了傳送門';
                break;
            case 'AvatarChange':
                text = `<strong>${noty.displayName}</strong> 變更角色為 ${noty.name}`;
                break;
            case 'Event':
                text = noty.data;
                break;
            case 'VideoPlay':
                text = `<strong>正在播放：</strong>${noty.notyName}`;
                break;
            case 'BlockedOnPlayerJoined':
                text = `已封鎖的用戶 <strong>${noty.displayName}</strong> 加入了世界`;
                break;
            case 'BlockedOnPlayerLeft':
                text = `已封鎖的用戶 <strong>${noty.displayName}</strong> 離開了世界`;
                break;
            case 'MutedOnPlayerJoined':
                text = `已靜音的用戶 <strong>${noty.displayName}</strong> 加入了世界`;
                break;
            case 'MutedOnPlayerLeft':
                text = `已靜音的用戶 <strong>${noty.displayName}</strong> 離開了世界`;
                break;
            case 'Blocked':
                text = `<strong>${noty.displayName}</strong> 封鎖了你`;
                break;
            case 'Unblocked':
                text = `<strong>${noty.displayName}</strong> 解除封鎖了你`;
                break;
            case 'Muted':
                text = `<strong>${noty.displayName}</strong> 靜音了你`;
                break;
            case 'Unmuted':
                text = `<strong>${noty.displayName}</strong> 解除靜音了你`;
                break;
            default:
                break;
        }
