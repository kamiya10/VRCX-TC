...
(async function () {
    ...
    API.call = function (endpoint, options) {
        ...
        var req = webApiService
            ...
            .then(({data, status}) => {
                ...
                if (status === 401 && data.error.message === '"缺少憑據"') {
    ...
    */
    ...
    $app.methods.playNotyTTS = function (noty, message) {
        switch (noty.type) {
            case 'OnPlayerJoined':
                this.speak(`${noty.displayName} 加入了房間`);
                break;
            case 'OnPlayerLeft':
                this.speak(`${noty.displayName} 離開了房間`);
                break;
            case 'OnPlayerJoining':
                this.speak(`${noty.displayName} 正在加入房間`);
                break;
            case 'GPS':
                this.speak(
                    `${noty.displayName} 正在 ${this.displayLocation(
                        noty.location,
                        noty.worldName
                    )}`
                );
            ...
            case 'Online':
                var locationName = '';
                if (noty.worldName) {
                    locationName = `在 ${this.displayLocation(
                        noty.location,
                        noty.worldName
                    )}`;
                }
                this.speak(`${noty.displayName} 已上線${locationName}`);
                break;
            case 'Offline':
                this.speak(`${noty.displayName} 已離線`);
                break;
            case 'Status':
                this.speak(
                    `${noty.displayName} 的狀態目前是 ${noty.status} ${noty.statusDescription}`
                );
                break;
            case 'invite':
                this.speak(
                    `${noty.senderUsername} 邀請你到 ${this.displayLocation(
                ...
                break;
            case 'requestInvite':
                this.speak(`${noty.senderUsername} 向你請求了邀請${message}`);
                break;
            case 'inviteResponse':
                this.speak(`${noty.senderUsername} 回覆了你的邀請${message}`);
                break;
            case 'requestInviteResponse':
                this.speak(
                    `${noty.senderUsername} 回覆了你的邀請請求${message}`
                );
                break;
            case 'friendRequest':
                this.speak(`${noty.senderUsername} 向你傳送了好友邀請`);
                break;
            case 'Friend':
                this.speak(`${noty.displayName} 現在是你的朋友了`);
                break;
            case 'Unfriend':
                this.speak(`${noty.displayName} 不再是你的朋友了`);
                break;
            case 'TrustLevel':
                this.speak(
                    `${noty.displayName} 的信認等級現在是 ${noty.trustLevel}`
                );
                break;
            case 'DisplayName':
                this.speak(
                    `${noty.previousDisplayName} 變更了他的名稱為 ${noty.displayName}`
                );
                break;
            case 'PortalSpawn':
                this.speak('用戶開啟了傳送門');
                break;
            case 'AvatarChange':
                this.speak(`${noty.displayName} 變更角色為 ${noty.name}`);
                break;
            case 'Event':
                this.speak(noty.data);
                break;
            case 'VideoPlay':
                this.speak(`正在播放：${noty.notyName}`);
                break;
            case 'BlockedOnPlayerJoined':
                this.speak(`已封鎖的用戶 ${noty.displayName} 加入了世界`);
                break;
            case 'BlockedOnPlayerLeft':
                this.speak(`已封鎖的用戶 ${noty.displayName} 離開了世界`);
                break;
            case 'MutedOnPlayerJoined':
                this.speak(`已靜音的用戶 ${noty.displayName} 加入了世界`);
                break;
            case 'MutedOnPlayerLeft':
                this.speak(`已靜音的用戶 ${noty.displayName} 離開了世界`);
                break;
            case 'Blocked':
                this.speak(`${noty.displayName} 封鎖了你`);
                break;
            case 'Unblocked':
                this.speak(`${noty.displayName} 解除封鎖了你`);
                break;
            case 'Muted':
                this.speak(`${noty.displayName} 靜音了你`);
                break;
            case 'Unmuted':
                this.speak(`${noty.displayName} 解除靜音了你`);
                break;
        }
    };
    ...
    $app.methods.displayXSNotification = function (noty, message, image) {
        ...
        switch (noty.type) {
            ...
            case 'Online':
                ...
                AppApi.XSNotification(
                    'VRCX',
                    `${noty.displayName} 已上線${locationName}`,
                    timeout,
                    image
                );
                break;
            ...
            case 'Status':
                AppApi.XSNotification(
                    'VRCX',
                    `${noty.displayName} 的狀態現在是 ${noty.status} ${noty.statusDescription}`,
                    timeout,
                    image
                );
                break;
            ...
            case 'requestInvite':
                AppApi.XSNotification(
                    'VRCX',
                    `${noty.senderUsername} 向你請求了邀請 ${message}`,
                    timeout,
                    image
                );
                break;
            case 'inviteResponse':
                AppApi.XSNotification(
                    'VRCX',
                    `${noty.senderUsername} 回覆了你的邀請 ${message}`,
                    timeout,
                    image
                );
                break;
            ...
            case 'AvatarChange':
                AppApi.XSNotification(
                    'VRCX',
                    `${noty.displayName} 變更角色為 ${noty.name}`,
                    timeout,
                    image
                );
                break;
    ...
    $app.methods.displayDesktopToast = function (noty, message, image) {
        switch (noty.type) {
            ...
            case 'OnPlayerLeft':
                ...
                break;
            case 'OnPlayerJoining':
                AppApi.DesktopNotification(noty.displayName, '正在加入', image);
                break;
            case 'GPS':
                AppApi.DesktopNotification(
                    noty.displayName,
                    `正在 ${this.displayLocation(
                        noty.location,
                        noty.worldName
                    )}`,
                    image
            ...
            case 'Online':
                ...
                break;
            case 'Offline':
                AppApi.DesktopNotification(noty.displayName, '已離線', image);
                break;
            case 'Status':
                AppApi.DesktopNotification(
                    noty.displayName,
                    `的狀態現在是 ${noty.status} ${noty.statusDescription}`,
                    image
                );
                break;
            case 'invite':
            ...
            case 'AvatarChange':
                ...
                break;
            case 'Event':
                AppApi.DesktopNotification('事件', noty.data, image);
                break;
            case 'VideoPlay':
                AppApi.DesktopNotification('正在播放', noty.notyName, image);
                break;
            case 'BlockedOnPlayerJoined':
                AppApi.DesktopNotification(
                    noty.displayName,
            ...
            case 'MutedOnPlayerLeft':
                ...
                break;
            case 'Blocked':
                AppApi.DesktopNotification(noty.displayName, '封鎖了你', image);
                break;
            case 'Unblocked':
                AppApi.DesktopNotification(
                    noty.displayName,
                ...
                break;
            case 'Muted':
                AppApi.DesktopNotification(noty.displayName, '靜音了你', image);
                break;
            case 'Unmuted':
                AppApi.DesktopNotification(
                    noty.displayName,
    ...
    $app.methods.promptTOTP = function () {
        this.$prompt('輸入你的身份驗證器應用程式中的數字代碼', '雙重認證', {
            distinguishCancelAndClose: true,
            cancelButtonText: '使用一次性密碼',
            confirmButtonText: '驗證',
            inputPlaceholder: '代碼',
            inputPattern: /^[0-9]{6}$/,
            inputErrorMessage: '無效的代碼',
    ...
    $app.methods.promptOTP = function () {
        this.$prompt('輸入你儲存的其中一個備份驗證碼', '雙重認證', {
            distinguishCancelAndClose: true,
            cancelButtonText: '使用限時一次性密碼',
            confirmButtonText: '驗證',
            inputPlaceholder: '代碼',
            ...
            AppApi.LoginWithSteam()
                ...
                .then((steamTicket) => {
                    if (steamTicket) {
                        ...
                        this.loginForm.loading = false;
                        this.$message({
                            message: '只有在 VRChat 開啟時才能使用這個選項',
                            type: 'error'
                        });
                    }
                });
    ...
    $app.methods.updateDiscord = function () {
            ...
            if (L.worldId) {
                ...
                switch (L.accessType) {
                    case 'public':
                        ...
                        L.accessName = `公開 #${L.instanceName} (${platform})`;
                        break;
                    case 'invite+':
                        L.accessName = `邀請+ #${L.instanceName} (${platform})`;
                        break;
                    case 'invite':
                        L.accessName = `邀請 #${L.instanceName} (${platform})`;
                        break;
                    case 'friends':
                        L.accessName = `好友 #${L.instanceName} (${platform})`;
                        break;
                    case 'friends+':
                        L.accessName = `好友+ #${L.instanceName} (${platform})`;
                        break;
        ...
        Discord.SetAssets(
            bigIcon, // big icon
            '由 VRCX 提供支援', // big icon hover text
    ...
    $app.methods.hideNotification = function (row) {
        this.$confirm(`你確定要拒絕 ${row.type} 嗎？`, '確定', {
            confirmButtonText: '確定',
            cancelButtonText: '取消',
            type: 'info',
            callback: (action) => {
    ...
    $app.methods.deleteNotificationLog = function (row) {
        this.$confirm(`你確定要刪除 ${row.type} 嗎？`, '確定', {
            confirmButtonText: '確定',
            cancelButtonText: '取消',
            type: 'info',
            callback: (action) => {
    ...
    $app.methods.promptUserIdDialog = function () {
        this.$prompt('輸入 用戶連結 或 用戶ID (UUID)', '直接存取', {
            distinguishCancelAndClose: true,
            confirmButtonText: 'OK',
            cancelButtonText: '取消',
            inputPattern: /\S+/,
            inputErrorMessage: '需要 用戶連結/用戶 ID',
            callback: (action, instance) => {
    ...
    $app.methods.promptUsernameDialog = function () {
        this.$prompt('輸入用戶名稱', '直接存取', {
            distinguishCancelAndClose: true,
            confirmButtonText: 'OK',
            cancelButtonText: '取消',
            inputPattern: /\S+/,
            inputErrorMessage: '需要用戶名稱',
            callback: (action, instance) => {
                if (action === 'confirm' && instance.inputValue) {
    ...
    $app.methods.promptWorldDialog = function () {
        this.$prompt('輸入 世界連結 或 世界ID (UUID)', '直接存取', {
            distinguishCancelAndClose: true,
            confirmButtonText: 'OK',
            cancelButtonText: '取消',
            inputPattern: /\S+/,
            inputErrorMessage: '需要 世界連結/世界 ID',
            callback: (action, instance) => {
                if (action === 'confirm' && instance.inputValue) {
                    var testUrl = instance.inputValue.substring(0, 15);
                    if (testUrl === 'https://vrchat.') {
    ...
    $app.methods.promptAvatarDialog = function () {
        this.$prompt('輸入 角色連結 或 角色 ID (UUID)', '直接存取', {
            distinguishCancelAndClose: true,
            confirmButtonText: 'OK',
            cancelButtonText: '取消',
            inputPattern: /\S+/,
            inputErrorMessage: '需要 角色連結/角色 ID',
            callback: (action, instance) => {
                if (action === 'confirm' && instance.inputValue) {
                    var testUrl = instance.inputValue.substring(0, 15);
                    if (testUrl === 'https://vrchat.') {
    ...
    $app.methods.promptNotificationTimeout = function () {
        this.$prompt('輸入秒數', '通知顯示時間', {
            distinguishCancelAndClose: true,
            confirmButtonText: 'OK',
            cancelButtonText: '取消',
            inputValue: this.notificationTimeout / 1000,
            inputPattern: /\d+$/,
            inputErrorMessage: '請輸入有效的秒數',
            callback: (action, instance) => {
                if (
                    action === 'confirm' &&
                    instance.inputValue &&
    ...
    $app.methods.promptRenameAvatar = function (avatar) {
        this.$prompt('輸入角色名稱', '重新命名角色', {
            distinguishCancelAndClose: true,
            confirmButtonText: 'OK',
            cancelButtonText: '取消',
            inputValue: avatar.ref.name,
            inputErrorMessage: '請輸入有效的名稱',
            callback: (action, instance) => {
                if (
                    action === 'confirm' &&
    ...
    $app.methods.promptChangeAvatarDescription = function (avatar) {
        this.$prompt('輸入角色敘述', '變更敘述', {
            distinguishCancelAndClose: true,
            confirmButtonText: 'OK',
            cancelButtonText: '取消',
            inputValue: avatar.ref.description,
            inputErrorMessage: '請輸入有效的敘述',
            callback: (action, instance) => {
                if (
                    action === 'confirm' &&
    ...
    $app.methods.promptChangeWorldCapacity = function (world) {
        this.$prompt('輸入世界最大人數, 最大：40', '變更世界最大人數', {
            distinguishCancelAndClose: true,
            confirmButtonText: 'OK',
            cancelButtonText: '取消',
    ...
    $app.methods.promptChangeWorldYouTubePreview = function (world) {
        this.$prompt('輸入世界 YouTube 預覽', '變更 YouTube 預覽', {
            distinguishCancelAndClose: true,
            confirmButtonText: 'OK',
            cancelButtonText: '取消',
            inputValue: world.ref.previewYoutubeId,
            inputErrorMessage: '請輸入有效的 Youtube 連結',
            callback: (action, instance) => {
                if (
                    action === 'confirm' &&
    ...
    $app.methods.promptAutoClearVRCXCacheFrequency = function () {
        this.$prompt(
            '輸入小時數，較大的數值可能會影響內存用量和性能 （預設：24，關閉：0）',
            'VRCX 快取清除間隔',
            {
                distinguishCancelAndClose: true,
                confirmButtonText: 'OK',
    ...
    $app.methods.promptSetAvatarRemoteDatabase = function () {
        this.$prompt('輸入角色資料庫提供方連結', '角色資料庫提供方', {
            distinguishCancelAndClose: true,
            confirmButtonText: 'OK',
            cancelButtonText: '取消',
            inputValue: this.avatarRemoteDatabaseProvider,
            inputPattern: /\S+/,
            inputErrorMessage: '請輸入有效的連結',
            callback: (action, instance) => {
                if (action === 'confirm' && instance.inputValue) {
    ...
    $app.methods.userDialogCommand = function (command) {
        ...
        if (command === 'Refresh') {
            ...
            this.$confirm(`你確定要 ${command} 嗎？`, '確定', {
                confirmButtonText: '確定',
                cancelButtonText: '取消',
                type: 'info',
                callback: (action) => {
            ...
            API.editInviteMessage(params, messageType, slot)
                ...
                .then((args) => {
                    ...
                    if (args.json[slot].message === D.inviteMessage.message) {
                        this.$message({
                            message: 'VRChat API 尚未更新訊息，請稍後再試',
                            type: 'error'
                        });
                        throw new Error(
    ...
    $app.data.VRChatConfigList = {
        cache_size: {
            name: '最大快取大小 [GB] (最小 20)',
            default: '20',
            type: 'number',
            min: 20
        },
        cache_expiry_delay: {
            name: '快取失效時長 [天] (30 - 150)',
            default: '30',
            type: 'number',
            min: 30,
            max: 150
        ...
        dynamic_bone_max_affected_transform_count: {
            name: '動態骨骼限制最大變換 (0 關閉所有變換)',
            default: '32',
            type: 'number',
            min: 0
        },
        dynamic_bone_max_collider_check_count: {
            name: '動態骨骼限制最大碰撞器碰撞 (0 關閉所有碰撞器)',
            default: '8',
            type: 'number',
            min: 0
        },
    ...
    $app.methods.readVRChatConfigFile = async function () {
        ...
        if (config) {
            try {
                ...
                this.$message({
                    message: '無效的 JSON 格式 (在 config.json)',
                    type: 'error'
                });
    ...
    $app.methods.showDeleteAllVRChatCacheConfirm = function () {
        this.$confirm(`你確定要刪除所有 VRChat 快取嗎？`, '確定', {
            confirmButtonText: '確定',
            cancelButtonText: '取消',
            type: 'info',
            callback: (action) => {
    ...
    $app.methods.showConsole = function () {
        ...
        console.log(
            '%c如果有人告訴您在這裡複製貼上東西，它們可能可以在不經你的許可下使用你的帳號。',
            'font-size: 20px;'
        );
    };
    ...
    $app.methods.ipcEvent = function (json) {
        ...
        try {
            ...
            console.error(`IPC 無效的 JSON，${json}`);
        }
        switch (data.type) {
            case 'OnEvent':
                if (this.debugPhotonLogging) {
